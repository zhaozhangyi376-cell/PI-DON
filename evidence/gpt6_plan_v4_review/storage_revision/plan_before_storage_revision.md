...
- [ ] N0 用 `shutil.disk_usage`、系统内存和 `nvidia-smi` 在 lab_log 内实测。#173旧值只用于预测：余5.91GiB、完整状态149.5MB；必须重新测。
- [ ] 预留 **2 GiB** 不用于正常产出，另预留 **512 MiB** 供失败raw/safe和紧急日志。正常可写上限=`启动空闲-2.5GiB`，每次保存前检查。预估本夜资产超过上限即不启动该分支。
- [ ] 实现新运行两槽 `checkpoint_A.pt/checkpoint_B.pt`：写临时文件→flush/fsync→原子替换非当前槽→读取验证hash和必要状态→原子更新小型commit指针。上一槽仍可恢复；提交清单append记录hash/sequence，不永久复制完整权重。
- [ ] 成功固定状态每点保留一份包含 H/E 的最终完整状态；失败保留当次raw/safe。单步中间预测/目标可另存小数组；轨迹登记快照只存六场/参考/metadata，不重复优化器。不得保留每10次更新的大模型副本。
- [ ] 旧 v1/v2/v3 目录只读。新目录正常滚动槽可被下一次正常提交原子替换；不得覆盖失败文件和验收最终文件。不要遍历磁盘删文件腾空间。
...
- [ ] 两槽机制以及真实torch.load的schema/hash校验完成，文件fsync后才提交指针；训练状态和记录器状态一起核验。不能只在内存复制payload称磁盘恢复。
...
- [ ] 每个已接受子步在实际参数提交后测得最终R<1e-4或合法零场规则；保存参数摘要hash、输入/targethash、计数和R以追溯，不以更新前loss判定。常规完整状态两槽滚动；所有实际保留的最终/失败/阶段checkpoint必须做磁盘纯推理回读，并另列R_disk。只存六场的轨迹快照不声称能回读对应模型，不把内存复算冒称每步都做过磁盘回读。
...
- [ ] 首次失败状态保存网络raw/safe、输入/target、分量/区域误差、实际预算与source/phase。只做纯前向定位，不新增原P4-B R1e-5训练分支，不再扫参。
...
- [ ] 每个阶段/任务一行：status、stop_reason、lab_run_id、起止UTC、source/config/input/checkpoint hash、origin/phase/source_index、actual Adam/head/solver/closure counts、loss_objective、R_disk（若实际回读）、physical_SSE/MSE、六分量MRE/nMAE/L2、A_fixed/Q、source-outside probes、可恢复资格、总资源。
...
```text
目标：请执行 C:/PI-DON/docs/superpowers/plans/2026-09-13-pidon-10h-goal-night-plan.md，在从首次执行开始累计不超过10小时的墙钟内，完成其中全部具备前置条件的工作，交付可逐项验收的中文证据包。先读 evidence/gpt6_plan_v4_review/REVIEW.md、audit.json、计划和实际源码，再按N0–N6推进。
...
第一次建立持久10小时deadline，恢复沿用同一deadline和全部累计配额；第9.5小时停止新增训练，预留最后30分钟验收。按计划修新运行两槽保存，留足磁盘底线，保留所有历史资产和新失败现场。所有出数运行包lab_log，逐阶段中文报告，记录真实Adam/head/求解/closure次数、最终物理R和实际磁盘回读、六分量、源外探针、source/phase和恢复资格。MRE/nMAE分栏，Yee/几何控制不得算DCO成绩。
