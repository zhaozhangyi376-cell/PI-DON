...
def component_metric(
    ...
    *,
    ...
    absolute_mae = weighted_abs / support_volume if support_volume > 0 else float("nan")
    ...
    return {
        ...
        "nmae": absolute_mae / denom_max if denom_max > 0 else float("nan"),
...
def six_component_metrics(
    ...
    *, source_ez_index: tuple[int, int, int] | None = None,
    ...
    result["metric_contract"] = {
        "nmae": "volume-weighted MAE/reference_max on the declared component support",
        "mre": "mre_nonzero only; strict zero reference points use absolute error",
