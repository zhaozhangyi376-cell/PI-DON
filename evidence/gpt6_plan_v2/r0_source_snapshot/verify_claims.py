...
def main():
    ...
    for cid, name, fn in DISK_CLAIMS + (STAGE2_CLAIMS if a.stage2_longrun else []) + (PIDON_V2_CLAIMS if a.pidon_v2 else []) + (PAPER_CLAIMS if a.paper_first else []) + (GRID_CLAIMS if a.grid_diagnosis else []) + (SPACING_CLAIMS if a.spacing_probe else []) + (COVERAGE_CLAIMS if a.coverage_ab else []) + (TRUNK_CLAIMS if a.trunk_repair else []):
        try:
            v, val, det = fn(None)
