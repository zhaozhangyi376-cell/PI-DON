...
class Solver:
    ...
    def inner_train(self, field, target, which="shared"):
        ...
        # A non-zero input with zero curl target receives a fixed scale from
        # the input magnitude and the cell length.  It is intentionally not a
        # scale extracted from the zero target.
        if den == 0.0:
            length = min(float(x) for x in self.d_mm) * 1e-3
            den = max(float(core.pow(2).sum()) / (length * length), 1e-30)
        ...
        def evaluate(with_grad=False):
            ...
            with context:
                ...
                sq = sum(sq_parts)
                if self.a.tol_mode == "rel" and getattr(self.a, "component_rel", False):
        ...
        stop_reason = "tol_met" if reached(initial_loss, self.a.tol) else "max_updates"
        while not reached(float(loss.detach()), self.a.tol) and n_updates < self.a.max_inner:
            ...
            if not all_finite(pred) or not torch.isfinite(train_loss):
                return FitRecord(n_updates, n_evals, initial_loss, value, float("nan"),
                                 float("nan"), physical_target_ss, False, "nonfinite",
            ...
            pred, _, loss = evaluate(with_grad=False)
            if not all_finite(pred) or not torch.isfinite(loss):
                return FitRecord(n_updates, n_evals, initial_loss, float("nan"), float("nan"),
                                 float("nan"), physical_target_ss, False, "nonfinite",
                                 time.perf_counter() - started, pred, tot, target_count)
        ...
        closure_budget = int(getattr(self.a, "lbfgs_closures", 0))
        lbfgs_budget_s = float(getattr(self.a, "lbfgs_time_budget_s", 0.0))
        if (not reached(float(loss.detach()), self.a.tol) and closure_budget > 0):
            ...
            def closure():
                ...
                return loss_c
            try:
                while (not reached(float(loss.detach()), self.a.tol) and n_closures < closure_budget
                       and (lbfgs_budget_s <= 0 or time.perf_counter() - started < lbfgs_budget_s)):
                    lbfgs.step(closure)
                    n_lbfgs_steps += 1
                    pred, _, loss = evaluate(with_grad=False)
                    if not all_finite(pred) or not torch.isfinite(loss):
                        return FitRecord(n_updates + n_lbfgs_steps, n_evals, initial_loss, float("nan"), float("nan"),
                                         float("nan"), physical_target_ss, False, "nonfinite",
                                         time.perf_counter() - started, pred, tot, target_count,
                                         n_lbfgs_steps, n_closures)
                ...
                return FitRecord(n_updates + n_lbfgs_steps, n_evals, initial_loss, float("nan"), float("nan"),
                                 ...
                                 time.perf_counter() - started, pred, tot, target_count,
                                 n_lbfgs_steps, n_closures)
            if not reached(float(loss.detach()), self.a.tol) and lbfgs_budget_s > 0 and time.perf_counter() - started >= lbfgs_budget_s:
                stop_reason = "time_budget"
        ...
        passed = reached(final_loss, self.a.tol)
        if passed:
            stop_reason = "tol_met"
    ...
    def state_payload(self):
        ...
        return {
            ...
            "E": [x.detach().cpu() for x in self.E],
            "H": [x.detach().cpu() for x in self.H],
            "net_H": self.net_H.state_dict(),
            "net_E": self.net_E.state_dict(),
            "opt_H": self.opt_H.state_dict(),
    ...
    def load_state_payload(self, payload):
        ...
        self.E = [x.to(self.dev) for x in payload["E"]]
        self.H = [x.to(self.dev) for x in payload["H"]]
    ...
    def step(self, g_t):
        ...
        if self.phase == "before_H":
            if getattr(self.a, "reset_opt_each_step", False):
...
def main():
    ...
    ap.add_argument("--strict-stop", action="store_true",
    ...
    ref = fdtd.PECCavity(side=a.side, n=a.n, dt=a.dt)
    if a.resume:
        loaded = torch.load(a.resume, map_location="cpu", weights_only=False)
        s.load_state_payload(loaded)
        _restore_reference(ref, loaded["reference"])
