...
def component_metric(
    ...
    *,
    ...
    return {
        ...
        "nmae": float(np.sum(mask * np.abs(delta)) / max(float(np.sum(mask)), 1.0) / denom_max) if denom_max > 0 else float("nan"),
        "absolute_mae": float(np.sum(mask * np.abs(delta)) / max(float(np.sum(mask)), 1.0)),
