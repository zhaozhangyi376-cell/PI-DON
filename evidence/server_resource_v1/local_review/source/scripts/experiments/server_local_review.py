...
def new_output(name, action):
    ...
    for rel in ('scripts/experiments/server_local_review.py',
                ...
                'src/pidon/pidon_solve.py', 'project_paths.py'):
        path = ROOT / rel
        if path.exists():
            dest = source / rel
...
def checkpoint_metrics(path, row):
    ...
    h = cfg['side'] / cfg['n']
    ref = ck['reference']
    measured = six_component_metrics(ck['E'], ck['H'], [ref[k] for k in NAMES[:3]],
                                     [ref[k] for k in NAMES[3:]], (h, h, h),
    ...
    for name in NAMES:
        for metric in ('absolute_mae', 'reference_max', 'nmae'):
            ...
            if b is not None and np.isfinite(a):
                diffs[f'{name}.{metric}'] = abs(a-b)
    return {'file': file_identity(path), 'accepted_steps': ck['accepted_steps'],
            'row_accepted_steps': row['accepted_steps'], 'max_abs_metric_difference': max(diffs.values(), default=0),
...
def run(args):
    ...
    out = new_output('local_review', args.action_id)
