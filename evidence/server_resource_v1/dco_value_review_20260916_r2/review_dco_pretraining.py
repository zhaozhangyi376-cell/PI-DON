...
def main():
    ...
    ap.add_argument("--import-directory", type=Path)
    args = ap.parse_args()
    ...
    imported = args.import_directory.resolve() if args.import_directory else out / "returned"
    imported.relative_to(BASE.resolve())
    if not args.import_directory:
        extract(args.zip, imported)
    rnd = imported / "random_low_lr128"
    summary, manifest = read(rnd / "summary.json"), read(rnd / "manifest.json")
    labs = jsonl(imported / "lab_runs.jsonl")
    ...
    events = [e for e in jsonl(imported / "actions.jsonl") if e.get("action_id") == summary["action_id"]]
    ...
    contract_path = ROOT / "src/pidon/pidon_contract.py"
    contract_lf = contract_path.read_bytes().replace(b"\r\n", b"\n")
    contract_lf_sha = hashlib.sha256(contract_lf).hexdigest()
    assert contract_lf_sha == manifest["source_hashes"]["pidon_contract.py"]
    (out / "pidon_contract_verified_lf.py").write_bytes(contract_lf)
    ...
    result = {"status": "PASS", "action_id": args.action_id, "lab_run_id": os.environ.get("PIDON_LAB_RUN_ID"),
              ...
              "metric_source_raw_sha256": sha(contract_path), "metric_source_lf_sha256": contract_lf_sha,
    ...
    lines = ["# DCO预训练价值：128步证据审计", "", f"本次仅读取、复算和绘图，参数更新为0。原始回传保存在`{imported.relative_to(ROOT)}`。", "",
             "| 指标 | 预训练初始化 | 随机初始化 |", "|---|---:|---:|"]
