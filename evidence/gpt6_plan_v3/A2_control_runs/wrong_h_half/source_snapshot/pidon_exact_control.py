...
import fdtd
from pidon_contract import (FitRecord, SOURCE_OUTSIDE_PROBES_CELLS,
...
def _reference_tensors(ref, solver, h_override=None):
    ...
    return ([to_t(getattr(ref, name), solver.dev, solver.dtype) for name in ("Ex", "Ey", "Ez")],
            [to_t(array, solver.dev, solver.dtype) for array in h_arrays])
...
def _metric_gate(result, threshold):
    return (result["finite"] and result["max_global_relative_l2"] <= threshold and
            result["max_probe_abs_error"] <= threshold)
...
def run(dtype, n=31, steps=128, side=0.05, dt=3.075e-12, mode="exact", *,
    ...
    for step, value in enumerate(source):
        ...
        if recorder is not None and resume_after is not None and solver.accepted_steps == resume_after:
            ...
            _restore_reference(reference, payload["reference"])
            ...
            restored_at = step + 1
    if recorder is not None:
        recorder.checkpoint(_checkpoint_payload(solver, reference, requested_steps=steps, source_index=steps))
