    ...
    def test_formal_resume_accepts_old_snapshot_and_replays_logged_step(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            OBS["stale_snapshot_resume"] = {
                ...
                "fixture": "zero source, exact-zero fast path, 0 Adam; nMAE display substituted only",
            }

    def test_known_snapshot_code_differences_are_newlines_only(self):
        snapshot = ROOT / "evidence/server_resource_v1/dco_value_review_20260916_r1/returned/random_low_lr128/source_snapshot"
...
if __name__ == "__main__":
    ...
    output = Path(__file__).with_name("integrity_counterexamples.json")
    with output.open("x", encoding="utf-8") as handle:
        json.dump({"review_only": True, "production_updates": 0, "tests": result.testsRun,
                   "failures": len(result.failures), "errors": len(result.errors),
                   "skipped": result.skipped, "observations": OBS}, handle, indent=2, ensure_ascii=False)
        handle.write("\n")
