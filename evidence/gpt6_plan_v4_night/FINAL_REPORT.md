...
本夜有限目标的证据包已完成，但论文第二阶段**未复现**：G0=PASS，G1=FAIL，G2=NOT_RUN，G3=NOT_RUN，论文复现状态=NOT_REPRODUCED。
...
| 阶段/门 | 状态 | 可验收证据 |
|---|---|---|
| N0 资产与资源 | PASS | 5 个历史资产哈希全部匹配；初始空闲 27217731584 bytes；清理 0 bytes |
| N1 合同与恢复 | PASS | 15/15 项 C01–C15；lab_log #202 |
| G0 测量认证 | PASS | 28/28 项；lab_log #204；仅 Yee/control |
| N3 / G1 | FAIL | 3 个登记顺序目标中有 2 个失败；lab_log #205/#206/#207 |
| N4 / G2 / G3 | NOT_RUN | G1 失败，按注册前置禁止进入长程轨迹 |
| N5 无更新诊断 | PASS | lab_log #212；正式 DCO 参数更新 0 |
| N6 证据汇总 | PASS | lab_log #217：本报告、JSON重算结果和图；#216：Y0–Y3结论表 |

## G0：已通过的工程认证（不计作 DCO）

完整 G0 为 28/28 PASS。float64 精确 Yee 128 步控制的 `Q=0`、`A_fixed=0`、源外探针绝对误差为 0；float32 对独立 float64 参考的最大 `Q=2.808238e-06`、`A_fixed=9.224699e-10`、探针误差 `8.429449e-09`，均通过其 `1e-4` 数值控制阈值。第 64 接受步确实从磁盘恢复，8192 步参考缓存哈希也通过。
...
|---|---:|---|---:|---:|---|
| H43 | 499 / 1 / 3 / 0 | max_updates | 0.000148940306 | 0.00014894104 | FAIL |
| H96 | 396 / 1 / 3 / 0 | tol_met | 9.98046877e-05 | 9.98047512e-05 | PASS |
| 顺序 E96 | 499 / 1 / 3 / 0 | max_updates | 0.000115438283 | 0.000115438304 | FAIL |

![N3 磁盘复测残差](/C:/PI-DON/evidence/gpt6_plan_v4_night/figs/n3_registered_disk_residuals_v3.png)
...
登记 G1 任务合计 Adam 更新 1394、head 提交 3、线性求解 9。另有 765 次 oracle-E Adam 更新完整保留但从 G1 排除。本夜证据实际占用 896967571 bytes，完成时可用空间 26280706048 bytes；初始空间足够，条件清理分支没有触发。
