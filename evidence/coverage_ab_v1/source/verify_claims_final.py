...
def main():
    ...
    results = []
    for cid, name, fn in DISK_CLAIMS + (PAPER_CLAIMS if a.paper_first else []) + (GRID_CLAIMS if a.grid_diagnosis else []) + (SPACING_CLAIMS if a.spacing_probe else []) + (COVERAGE_CLAIMS if a.coverage_ab else []):
        try:
            v, val, det = fn(None)
...
def write_md(results, ran):
    ...
    flag=('--coverage-ab' if 'T0' in ids else '--spacing-probe' if 'S0' in ids
