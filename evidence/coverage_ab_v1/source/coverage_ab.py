...
def train_arm(arm,data,initial,batches,root,device):
    ...
    hist=dict(arm=arm,updates=[],config=dict(lr=1e-4,batch=4,max_updates=200,loss='per-sample relative MSE',
                                          spacing_mm=RANGES[arm],initial_sha256=sha256(initial)))
    ...
    if len(batches)!=200 or min(steps)!=200 or max(steps)!=200:raise AssertionError('update budget mismatch')
    path=root/(arm+'.pt')
...
def evaluate(models,root,device):
    arrays={};rows=[];probe_rows=[];test_specs=[P.wave_spec(s) for s in [10,11,12]]
    tests=[]
    ...
    probes=[case for case in S.cases() if case['family']=='trunk_only']
    ...
    for model in models:
        ...
        print('Evaluated',model['tag'],'12 independent-seed cases and 30 coordinate probes',flush=True)
        del net
        if device=='cuda':torch.cuda.empty_cache()
