...
def require_metadata(ck):
    if ck.get('coords') != 'cellsize' or ck.get('norm') != 'rms' or ck.get('head') != 'direct':
...
def run(args):
    ...
    result['environment'] = {'torch': str(torch.__version__), 'numpy': np.__version__,
                             'cuda_runtime': torch.version.cuda, 'device': str(device),
                             'gpu': torch.cuda.get_device_name() if device.type == 'cuda' and torch.cuda.is_available() else None}
    start = time.perf_counter()
    try:
