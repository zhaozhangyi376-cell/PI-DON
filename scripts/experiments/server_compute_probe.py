...
def run(args):
    ...
    try:
        ...
        result['environment'] = {'cpu_logical_count': os.cpu_count(), 'torch_threads': torch.get_num_threads(),
                                 'cuda_runtime': torch.version.cuda, 'cudnn': torch.backends.cudnn.version(),
                                 'gpu': torch.cuda.get_device_name() if device.type == 'cuda' else None,
                                 'capability': list(torch.cuda.get_device_capability()) if device.type == 'cuda' else None,
        ...
        for micro in (4, 8, 16):
            ...
            try:
                ...
                try:
                    ...
                    row['save_error'] = repr(save_error)
                atomic_json_save(result, out / 'progress.json')
        ...
        result['status'] = 'PASS' if qualified and not result['unknown_tail'] and not any('save_error' in r for r in result['arms']) else 'INCOMPLETE'
    except Exception as exc:
