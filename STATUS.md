...
用户批准按 [20260917代码审查](evidence/code_review_20260917/REVIEW.md) 修改代码。修复清单、逐条影响边界和未决事项见
[修复记录](evidence/code_review_20260917_fixes/FIX_LOG.md)。要点：

- **0 次参数更新，没有重新训练，没有改写任何历史证据、权重或旧 FAIL。**
- 真空配置的求解系数逐位不变（`dt/EPS0`），因此现有 64/128 轨迹的数值解释不受本次修复影响。
- 修好的主要是「验收器能接受不完整记录」「恢复合同不完整」「成本记账重复或归零」三类问题，
  不是某个已被证实导致当前 P/R128 场误差的计算错误。**代码漏洞与实验失败原因仍是两件事。**
- 旧 `PASS` 的适用范围随之收紧（场门要求六分量齐全并纳入源外探针、回传审核不再复述摘要自身状态），
  但**已明确的 Ex/Ey FAIL 和其它旧 FAIL 一律不改判**。
- 已定位到具体旧解释的三处更正：E01 的 `0.787333` 不能量化非线性；E02 的 `831.6` 倍不是同目标比较；
  汇报 r4 第 26 页的 32³ `2.791e-3` 属于 `lr1e3_300` 而非 `paper32`（paper32 同尺寸为 `4.738e-3`）。
  **原始数值全部保留**，改的是比较口径和模型归属。
- 兼容性提醒：`protocol_hash` 的冻结清单从 5 个源文件扩大到 9 个实际依赖，
  因此**修复前开始、尚未结束的正式运行不能在修复后的代码下 `--resume`**，应在其冻结快照下跑完或登记新实验重开。
- 测试：根目录 220 项、2 个错误（均为本机缺失的 `.pt` 证据文件，修复前同样存在）；`_01` 8 项通过。
  新增 `tests/test_code_review_20260917_fixes.py` 共 64 项负面/边界测试。测试绿色不认证科学 PASS。
...
- S1原计划文字为“逐样本三分量宏nMAE≤1%”，当前代码使用“样本宏nMAE的均值≤1%”。**已修复（F21）**：场门同时输出均值归约与逐样本归约，均值仍是登记判据；不能用较宽判据升级旧失败。
- S1保存了全局RNG，没有保存用于randperm的独立torch.Generator状态。**已修复（F21）**：`recovery_eligible`由硬编码True改为False并附逐项`recovery_contract`；完成旧25000预算仍不产生追加资格。
- 原M2审计field_gate_pass没有覆盖全部Q/弱分量检查。**已修复（F02补）**：缺分量、非有限nMAE、未检查的Q和弱场绝对门全部纳入，缺项记INCOMPLETE。当前时刻弱参考标志仍不等于登记的全时长固定参考规则。
- M2运行器保存64快照，但未在该节点执行完整场门停止逻辑。**部分修复（F02）**：新增全窗口场门与源外整段波形检查，科学PASS需端点门和窗口门同时通过；已有128轨迹仍不能未经审核称为“按全部检查点合同合格”。
- 上述修复**不改变任何旧FAIL**；新正式实验仍须在修复后的测量/恢复合同下重新登记，不靠旧布尔字段解锁长程。修复清单见[FIX_LOG](evidence/code_review_20260917_fixes/FIX_LOG.md)。
...
| CODE-REVIEW-FIX | 交付PASS，0更新 | 按20260917审查实施生产修复：物理/测量合同、验收器、恢复与记账、第一阶段比较合同；新增64项负面测试；未重训、未改写历史证据、未改判任何旧FAIL |
