...
// 19
{
  ...
  table(s, [
    ...
    // PR02（审查20260917）：项目 R 是内层优化的停止量，论文 9.6e-3 是时域场
    // 预测的 MRE，两者既不同量也不同阶段；R 与论文 Eq.(7) loss 的单位/归约
    // 等价关系仍未确认，因此不能计算“严格多少倍”。
    ['严格R<1e-5','控制长期误差','R是停止量，论文9.6e-3是场MRE，等价关系未确认，不算倍数','论文阈值另线诊断'],
...
// 26
{
  ...
  table(s, [
    // PR04（审查20260917）：32³ 格原来填 2.791e-3，那是
    // test_results_dco_lr1e3_300.npz:exp2_nmae[1] 的值，属于 lr1e3_300 这个
    // checkpoint；同尺寸的 paper32 实际为 test_results_dco_paper32.npz:
    // exp2_nmae[0] = 4.738e-3。原始数值都没变，改的是模型归属。
    ['网格','论文 MRE','旧 paper32 nMAE','旧 lr1e3_300 nMAE','S1R blind nMAE','判断'],
    ['32³','L4: 7.7e-4','4.738e-3','2.791e-3','4.150e-2','指标口径不同'],
    ['64³','4.1e-3','2.875e-3','—','数据已回传','旧模型较好'],
    ['64×96×16','3.8e-3','5.687e-3','—','数据已回传','旧模型略差'],
    ['32×64×16','4.7e-3','6.266e-3','—','数据已回传','旧模型略差'],
  ], 0.65, 1.25, 12.0, 3.3, [1.8,1.9,2.2,2.2,2.2,1.7], 11);
...
// 29
{
  ...
  // PR14（审查20260917）：H 只有 Hx/Hy/Hz 三个分量；"六个"是原来的固定文字。
  box(s, 0.7, 1.25, 3.2, 1.35, '当前状态', 'Hⁿ⁺¹ᐟ² 的三个 Yee 支撑数组', C.blue, { bodySize: 13 });
