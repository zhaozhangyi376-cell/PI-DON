"""Install the SR-MICRO16 payload and run its queue."""
...
def main() -> int:
    ...
    backup = ROOT / "evidence/server_resource_v1" / (
        "micro16_deployment_backup_" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    )
    ...
    print(f"Installed SR-MICRO16 payload. Backup: {backup if backup.exists() else 'none'}")
    subprocess.run([sys.executable, "run.py", "unittest", "-q", "test_first_e_budget_probe"], cwd=ROOT, check=True)
    return subprocess.run([sys.executable, "run.py", "server_micro16_queue"], cwd=ROOT, check=False).returncode
