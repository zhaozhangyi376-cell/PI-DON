"""Import and summarize returned SR-64-LOWLR-RANDOM server evidence."""
...
def make_report(import_dir: Path, probe_dir: Path, summary: dict[str, Any], rows: list[dict[str, Any]]) -> Path:
    ...
    report = ROOT / "random_low_lr64_return_review.md"
    report.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return report
...
def update_plan(probe_dir: Path, report: Path, summary: dict[str, Any]) -> None:
    ...
    task["summary"] = (
        f"Random low-lr64 diagnostic returned {summary.get('status')}; "
