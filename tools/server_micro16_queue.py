"""Install-time runner for the SR-MICRO16 server probe."""
...
OUTPUT = "evidence/server_resource_v1/micro16_strict_probe"
...
def ensure_task():
    ...
    if TASK not in tasks:
        plan["tasks"].append({
            ...
            "status": "READY",
            "depends": ["SR-MICRO4"],
            "reason": "检验9000 Adam半步预算能否从4步稳定扩展到16个完整严格步",
            "evidence": [],
            "kind": "delivery",
...
def main() -> int:
    ...
    if action is None:
        ...
        start = subprocess.run([
            ...
            "--question", "SR-MICRO16: can a strict tol=1e-5 online trajectory complete 16 full steps with 9000 Adam per half-step?",
            "--expected", "Run only 16 full steps from the old lr1e3 DCO checkpoint; write field metrics and failure现场; do not start 64/128/1024",
            "--success", "16 accepted steps plus step-16 micro field gate pass, or a preserved FAIL/RESOURCE_LIMIT/INCOMPLETE现场",
        ...
        action = active_action()
    if action is None:
        print("Could not resolve active SR-MICRO16 action.", file=sys.stderr)
        return 2
    cmd = [
        sys.executable, "lab_log.py", "run",
        "-m", "SR-MICRO16 strict 16-step online probe",
        "--", sys.executable, "run.py", "--action", action,
        "server_short_tol_probe", "--action-id", action,
        "--device", "cuda",
        "--tol", "1e-5",
        "--target-steps", "16",
        "--output-name", "micro16_strict_probe",
        "--per-fit-cap", "9000",
        "--overall-adam-cap", "288000",
        "--micro-gate-steps", "16",
        "--protocol", PROTOCOL,
    ]
    ...
    if summary_path.exists():
        ...
        summary_text = (
            f"SR-MICRO16 delivery {status}; scientific_result={summary.get('scientific_result')}; "
