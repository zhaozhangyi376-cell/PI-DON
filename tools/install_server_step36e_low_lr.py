"""Install the SR-36E-LOWLR payload and run its queue."""
...
def main() -> int:
    ...
    backup = ROOT / "evidence/server_resource_v1" / (
        "step36e_low_lr_deployment_backup_" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    )
    ...
    print(f"Installed SR-36E-LOWLR payload. Backup: {backup if backup.exists() else 'none'}")
    py_compile.compile(str(ROOT / "scripts/experiments/server_step36e_budget_probe.py"), doraise=True)
    py_compile.compile(str(ROOT / "tools/server_step36e_low_lr_queue.py"), doraise=True)
    return subprocess.run([sys.executable, "run.py", "server_step36e_low_lr_queue"], cwd=ROOT, check=False).returncode
