"""Import and summarize returned SR-128 server evidence."""
...
def find_probe(base: Path) -> Path:
    matches = [
        p
        for p in base.rglob("strict128_probe")
        if p.is_dir() and (p / "summary.json").exists()
    ]
    if len(matches) != 1:
        raise SystemExit(f"Expected one strict128_probe/summary.json, found {len(matches)}")
    return matches[0]
...
def make_report(import_dir: Path, probe_dir: Path, summary: dict, rows: list[dict]) -> Path:
    ...
    conclusion = (
        "128步场门通过；可在本地审计后另行登记1024步验证，仍不能直接启动8192。"
        if summary.get("scientific_result") == "PASS_128"
        else "128步未通过或未完成；停止该分支，不启动1024/8192。"
    )
    lines = [
        "# SR-128 回传审计",
        "",
    ...
    report = ROOT / "strict128_return_review.md"
    report.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return report
...
def update_plan(probe_dir: Path, report: Path, summary: dict):
    ...
    task["summary"] = (
        f"128-step strict probe returned {summary.get('status')}; "
    ...
    for gap in plan.get("gaps", []):
        if gap.get("id") == "SHORT":
            gap["current"] = (
                "SR-128回传PASS_128：128步严格场门通过；仍需另行登记1024。"
                if pass128
                else "SR-128回传未达PASS_128；不得启动1024/8192。"
            )
            ...
            gap["current"] = "SR-128不自动解锁1024/8192；本地审计后才可登记1024"
            gap["status"] = "NOT_RUN"
        elif gap.get("id") == "PROVENANCE":
            gap["current"] = "服务器SR-128证据已回传；新旧失败现场保留"
            gap.setdefault("evidence", [])
            if relative(report) not in gap["evidence"]:
                gap["evidence"].append(relative(report))
