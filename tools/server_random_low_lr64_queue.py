"""Install-time runner for the random-init clean low-lr 64-step diagnostic."""
...
TASK = "SR-64-LOWLR-RANDOM"
PROTOCOL = "docs/plans/2026-09-16-random-low-lr-64-protocol.md"
...
def ensure_task():
    ...
    task = {
        ...
        "title": "随机初始化低学习率64步对照",
        "status": "READY",
        "depends": [],
        "reason": "与clean low-lr64同规则比较随机初始化和预训练DCO的成本与场门",
        "evidence": [],
        "kind": "diagnostic",
        "scientific_result": "NOT_RUN",
        "execution_plan": PROTOCOL,
        "summary": "random init; lr=1e-4, tol=1e-5, max_inner=60000, target=64.",
    }
...
def main() -> int:
    ...
    if action is None:
        ...
        start = subprocess.run([
            ...
            "--question", "SR-64-LOWLR-RANDOM: can random init pass the same clean low-lr64 field gate?",
            "--expected", "Run one random-init 64-step trajectory with lr=1e-4 and tol=1e-5; write all evidence; do not start 128 automatically",
            "--success", "64-step diagnostic evidence with cost and field gates, or preserved FAIL/RESOURCE_LIMIT/INCOMPLETE evidence",
        ...
        action = active_action()
    if action is None:
        print("Could not resolve active SR-64-LOWLR-RANDOM action.", file=sys.stderr)
        return 2
    cmd = [
        sys.executable, "lab_log.py", "run",
        "-m", "SR-64-LOWLR-RANDOM random-init strict 64-step field diagnostic",
    ...
    if summary_path.exists():
        ...
        summary_text = (
            f"SR-64-LOWLR-RANDOM delivery {status}; scientific_result={summary.get('scientific_result')}; "
