"""Install-time runner for the paper-tolerance 128-step diagnostic."""
...
def ensure_task():
    ...
    task = {
        ...
        "summary": "diagnostic trajectory from step0; lr=1e-4, tol=1e-4, max_inner=60000, target=128.",
    }
...
def main() -> int:
    ...
    if action is None:
        ...
        start = subprocess.run([
            ...
            "--question", "SR-128-PAPER-TOL-DIAG: what cost and field error result from lr=1e-4 with paper-like tol=1e-4 to 128 steps?",
            "--expected", "Run one diagnostic 128-step trajectory from the DCO checkpoint with lr=1e-4 and tol=1e-4; write all evidence; do not start 1024 automatically",
        ...
        action = active_action()
    if action is None:
        print("Could not resolve active SR-128-PAPER-TOL-DIAG action.", file=sys.stderr)
        return 2
    cmd = [
        sys.executable, "lab_log.py", "run",
        "-m", "SR-128-PAPER-TOL-DIAG clean low-lr 128-step field gate",
    ...
    if summary_path.exists():
        ...
        summary_text = (
            f"SR-128-PAPER-TOL-DIAG delivery {status}; scientific_result={summary.get('scientific_result')}; "
