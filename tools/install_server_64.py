"""Install the SR-64 payload and run its queue."""
...
def main() -> int:
    ...
    backup = ROOT / "evidence/server_resource_v1" / (
        "strict64_deployment_backup_" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    )
    ...
    print(f"Installed SR-64 payload. Backup: {backup if backup.exists() else 'none'}")
    subprocess.run([sys.executable, "run.py", "unittest", "-q", "test_first_e_budget_probe"], cwd=ROOT, check=True)
    return subprocess.run([sys.executable, "run.py", "server_64_queue"], cwd=ROOT, check=False).returncode
