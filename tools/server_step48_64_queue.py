"""Install-time runner for the SR-48-64 low-lr local window."""
...
OUTPUT = "evidence/server_resource_v1/step48_64_low_lr_window"
...
def ensure_task():
    ...
    task = {
        ...
        "status": "READY",
        "depends": [],
        "reason": "验证35→48通过后，低学习率分支能否继续达到64步局部门槛",
        "evidence": [],
        "kind": "diagnostic",
        "scientific_result": "NOT_RUN",
        "execution_plan": PROTOCOL,
        "summary": "从step36_48_low_lr_window干净48步checkpoint继续到64；lr=1e-4；不解锁长程。",
    }
...
def main() -> int:
    ...
    if action is None:
        ...
        start = subprocess.run([
            ...
            "--question", "SR-48-64-LOWLR: can the low-lr local branch continue from accepted step 48 to accepted step 64?",
            "--expected", "Run a bounded local continuation from the clean step48 checkpoint; preserve failure evidence; do not claim formal from-zero 64/128 pass",
            "--success", "Diagnostic PASS/FAIL with summary/report/manifest/steps/checkpoint/failure evidence",
            "--failure", "Stop at first failure; do not widen the window or start 128/1024/8192 from this action",
            "--protocol", PROTOCOL,
        ...
        action = active_action()
    if action is None:
        print("Could not resolve active SR-48-64-LOWLR action.", file=sys.stderr)
        return 2
    cmd = [
        sys.executable, "lab_log.py", "run",
        "-m", "SR-48-64-LOWLR local continuation from step48",
        ...
        "--target-accepted-steps", "64",
        "--max-inner", "60000",
        "--lr", "1e-4",
        "--fit-progress-every", "500",
        "--output-name", "step48_64_low_lr_window",
    ]
    ...
    if summary_path.exists():
        ...
        summary_text = (
            f"SR-48-64-LOWLR delivery {status}; scientific_result={summary.get('scientific_result')}; "
