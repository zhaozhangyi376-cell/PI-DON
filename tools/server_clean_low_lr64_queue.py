"""Install-time runner for the clean low-lr 64-step probe."""
...
TASK = "SR-64-LOWLR-CLEAN"
PROTOCOL = "docs/plans/2026-09-16-clean-low-lr-64-protocol.md"
OUTPUT = "evidence/server_resource_v1/clean_low_lr64"
...
def ensure_task():
    ...
    task = {
        ...
        "status": "READY",
        "depends": [],
        "reason": "混合历史低学习率局部窗口已到64，需验证同一低学习率能否从0干净达到64场门",
        "evidence": [],
        "kind": "delivery",
        "scientific_result": "NOT_RUN",
        "execution_plan": PROTOCOL,
        "summary": "clean trajectory from step0; lr=1e-4, tol=1e-5, max_inner=60000, target=64.",
    }
...
def main() -> int:
    ...
    if action is None:
        ...
        start = subprocess.run([
            ...
            "--question", "SR-64-LOWLR-CLEAN: can lr=1e-4 reach 64 accepted steps from step0 and pass the 64 field gate?",
            "--expected", "Run one clean 64-step trajectory from the DCO checkpoint with lr=1e-4; write all evidence; do not start 128/1024 automatically",
            "--success", "64 accepted steps and registered 64 field gate PASS, or preserved FAIL/RESOURCE_LIMIT/INCOMPLETE evidence",
        ...
        action = active_action()
    if action is None:
        print("Could not resolve active SR-64-LOWLR-CLEAN action.", file=sys.stderr)
        return 2
    cmd = [
        sys.executable, "lab_log.py", "run",
        "-m", "SR-64-LOWLR-CLEAN clean low-lr 64-step field gate",
    ...
    if summary_path.exists():
        ...
        summary_text = (
            f"SR-64-LOWLR-CLEAN delivery {status}; scientific_result={summary.get('scientific_result')}; "
