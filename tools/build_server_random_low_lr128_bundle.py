"""Build the SR-128-LOWLR-RANDOM server deployment zip."""
...
PAYLOAD = [
    "docs/plans/2026-09-16-random-low-lr-128-protocol.md",
    "scripts/experiments/server_short_tol_probe.py",
    "tools/server_random_low_lr128_queue.py",
...
with zipfile.ZipFile(ROOT / "server_random_low_lr128_bundle.zip") as zf:
...
print("Installed SR-128-LOWLR-RANDOM payload. Backup: none")
print(subprocess.run([sys.executable, "-m", "unittest", "tests.test_first_e_budget_probe", "-v"], cwd=ROOT, text=True).returncode)
raise SystemExit(subprocess.run([sys.executable, "run.py", "server_random_low_lr128_queue"], cwd=ROOT).returncode)
'''


def main() -> None:
    out = PROJECT_DIR / "evidence/server_resource_v1/server_random_low_lr128_bundle.zip"
    out.parent.mkdir(parents=True, exist_ok=True)
    if out.exists():
        out.unlink()
    manifest = {"schema": "pidon-server-random-low-lr128-bundle-v1", "payload": {}}
    with zipfile.ZipFile(out, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.writestr("install_server_random_low_lr128.py", INSTALL)
